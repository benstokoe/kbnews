...
const Comment = ({
  ...
  const handleBan = (): Promise<void> => {
    return;
  };

  const handlePin = (): Promise<void> => {
    return;
  };
    ...
    <div className="flex flex-col">
      {!hidden && parent && (
        <div className="grid pb-1 gap-x-2 comment-grid">
          ...
          <div className="col-start-2 flex items-center leading-none mb-1 transform translate-y-1">
            <button
              className="text-xs hover:underline focus-ring active:underline cursor-pointer focus:outline-none"
              aria-label={`View comment by ${parent.author?.full_name}`}
            >
              @{parent.author?.full_name}:
            </button>
            <div className="text-xs  ml-1 hover:text-gray-400 focus-ring active:text-gray-400 cursor-pointer focus:outline-none line-clamp-1">
              {parent.content}
            </div>
          </div>
        </div>
      ...
      >
        ...
        {!hidden ? (
          ...
          >
            <BsPlusCircle className="w-4 h-4 text-secondary" />
          </button>
        )}
        <div className="row-start-1 col-start-2 self-center">
          <div className="flex flex-grow items-end">
            ...
            <span className="font-semibold text-xs mx-1 leading-none select-none">
              ·
            </span>
            <span
            ...
            {isAdmin && (
              <button
                className="text-xs flex flex-row items-center focus-ring border-none ml-5 leading-none"
                onClick={handlePin}
        ...
        <div className={cn("row-start-2 col-start-2", { hidden })}>
          <section
            className={cn("leading-6 text-sm", {
          ...
          {!comment.isDeleted && (
            <div className="grid grid-flow-col auto-cols-min gap-x-3 transform -translate-x-1.5">
              ...
              <button
                className="text-xs flex items-center focus-ring border-none"
